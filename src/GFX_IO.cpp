...
// Possible values for GFX_IO.connection:
...
/*!
    @brief   GFX_IO constructor for software (bitbang) SPI.
...
/*!
    @brief   GFX_IO constructor for hardware SPI using the board's
...
/*!
    @brief   GFX_IO constructor for hardware SPI using a specific
             SPI peripheral.
...
/*!
    @brief   GFX_IO constructor for parallel display connection.
    @param   w         Display width in pixels at default rotation (0).
    @param   h         Display height in pixels at default rotation (0).
...
*/
void GFX_IO::initSPI(uint32_t freq, uint8_t spiMode) {
...
*/
void GFX_IO::setSPISpeed(uint32_t freq) {
#if defined(SPI_HAS_TRANSACTION)
...
*/
void GFX_IO::writePixel(int16_t x, int16_t y, uint16_t color) {
...
*/
void GFX_IO::writeColor(uint16_t color, uint32_t len) {
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void GFX_IO::drawPixel(int16_t x, int16_t y, uint16_t color) {
  // Clip first...
  if ((x >= 0) && (x < _width) && (y >= 0) && (y < _height)) {
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void GFX_IO::pushColor(uint16_t color) {
  startWrite();
  SPI_WRITE16(color);
  endWrite();
}
...
*/
void GFX_IO::invertDisplay(bool i) {
  startWrite();
...
*/
uint16_t GFX_IO::color565(uint8_t red, uint8_t green, uint8_t blue) {
...
*/
inline void GFX_IO::SPI_BEGIN_TRANSACTION(void) {
  if (connection == TFT_HARD_SPI) {
#if defined(SPI_HAS_TRANSACTION)
...
*/
inline void GFX_IO::SPI_END_TRANSACTION(void) {
#if defined(SPI_HAS_TRANSACTION)
  if (connection == TFT_HARD_SPI) {
...
*/
void GFX_IO::spiWrite(uint8_t b) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(b);
#elif defined(ESP8266) || defined(ESP32)
...
*/
void GFX_IO::writeCommand(uint8_t cmd) {
  SPI_DC_LOW();
  spiWrite(cmd);
  SPI_DC_HIGH();
}
...
*/
void GFX_IO::write16(uint16_t w) {
  if (connection == TFT_PARALLEL) {
#if defined(USE_FAST_PINIO)
    if (tft8.wide)
...
*/
void GFX_IO::writeCommand16(uint16_t cmd) {
  SPI_DC_LOW();
  write16(cmd);
  SPI_DC_HIGH();
}
...
*/
inline void GFX_IO::SPI_MOSI_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.mosiPortSet = 1;
...
*/
inline void GFX_IO::SPI_MOSI_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.mosiPortClr = 1;
...
*/
inline void GFX_IO::SPI_SCK_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.sckPortSet = 1;
...
*/
inline void GFX_IO::SPI_SCK_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.sckPortClr = 1;
...
*/
inline bool GFX_IO::SPI_MISO_READ(void) {
#if defined(USE_FAST_PINIO)
#if defined(KINETISK)
...
/*!
    ...
    @param  w  16-bit value to write.
*/
void GFX_IO::SPI_WRITE16(uint16_t w) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(w >> 8);
    AVR_WRITESPI(w);
...
*/
void GFX_IO::SPI_WRITE32(uint32_t l) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(l >> 24);
    AVR_WRITESPI(l >> 16);
...
*/
inline void GFX_IO::TFT_WR_STROBE(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *tft8.wrPortClr = 1;
...
/*!
    @brief  Set the RD line HIGH. Used for parallel-connected interfaces
            when reading data.
*/
inline void GFX_IO::TFT_RD_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
...
/*!
    @brief  Set the RD line LOW. Used for parallel-connected interfaces
            when reading data.
*/
inline void GFX_IO::TFT_RD_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
